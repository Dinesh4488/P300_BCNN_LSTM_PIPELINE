...
If you want to version large binaries such as model checkpoints (`*.pt`) or arrays (`*.npy`) efficiently, install and enable Git LFS before committing large files:
...
Note: this repository includes `results_A/` and `results_B/` as reference results. If you plan to add or update checkpoints, enable Git LFS locally to avoid oversized pushes.

Stopping tracking regenerable files (if previously committed)

If you previously committed regenerable files (for example `*.pt` or `*.npy`) and want to stop tracking them while keeping the local files, run the following commands from the repository root:

```pwsh
# 1) ensure .gitignore contains the patterns (already done)
git add .gitignore
git commit -m "Update .gitignore to ignore regenerable outputs"

# 2) remove tracked regenerable files from git index but keep them on disk
git rm --cached -r -- "*.pt" "*.npy" "data/processed" || true
git commit -m "Stop tracking regenerable outputs (checkpoints, arrays, processed data)"

# 3) push changes
git push
```

Note: these commands stop tracking the files in future commits but do not remove them from existing Git history if they were previously committed. To remove large files from history, use a history-rewrite tool (BFG or `git filter-repo`) — see the section below.

---
Module developed by the project owner. For questions or help reproducing results, open an issue or contact the maintainer.
